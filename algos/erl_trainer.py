...
class ERL_Trainer:
	...
	def __init__(self, args, env_constructor):

		self.args = args
		self.manager = Manager() # 共享进程间数据
		...
		for _ in range(args.pop_size):
			self.population.append(CategoricalPolicy(env_constructor.n_s, env_constructor.n_a, args.hidden_size))

		#Save best policy
		self.best_policy = CategoricalPolicy(env_constructor.n_s, env_constructor.n_a, args.hidden_size)

		#PG Learner


		self.learner = DDQN(args, env_constructor)
		...
		for _ in range(args.rollout_size):
			self.rollout_bucket.append(CategoricalPolicy(env_constructor.n_s, env_constructor.n_a, args.hidden_size))

		############## MULTIPROCESSING TOOLS ###################
		#Evolutionary population Rollout workers
		self.evo_task_pipes = [Pipe() for _ in range(args.pop_size)]
		self.evo_result_pipes = [Pipe() for _ in range(args.pop_size)]
		self.evo_workers = [Process(target=rollout_worker, args=(id, 'evo', self.evo_task_pipes[id][1], self.evo_result_pipes[id][0],
																 args.rollout_size > 0, self.population)) for id in range(args.pop_size)]
		...
		self.workers = [Process(target=rollout_worker, args=(id, 'pg', self.task_pipes[id][1], self.result_pipes[id][0],
															 True, self.rollout_bucket))
		...
		self.test_bucket.append(CategoricalPolicy(env_constructor.n_s, env_constructor.n_a, args.hidden_size))

		# Test workers
		self.test_task_pipes = [Pipe() for _ in range(args.num_test)]
		self.test_result_pipes = [Pipe() for _ in range(args.num_test)]
		self.test_workers = [Process(target=rollout_worker, args=(id, 'test', self.test_task_pipes[id][1], self.test_result_pipes[id][0], False, self.test_bucket)) for id in range(args.num_test)]
	...
	def forward_generation(self, gen, tracker):
		...
		self.learner.actor.cpu() # learner的q_network
