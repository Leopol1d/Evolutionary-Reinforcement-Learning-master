...
def train(args):
    ...
    my_al = My_AL(env=env, env_eval=env_eval, memory_capacity=MEMORY_CAPACITY,
                  ...
                  pop_size=100, rollout_size=25,dirs=dirs)
    my_al.train(1)
...
def evaluate(args):
    ...
    my_al = My_AL(env=env, env_eval=env_eval, memory_capacity=MEMORY_CAPACITY,
                  ...
                  max_grad_norm=MAX_GRAD_NORM, test_seeds=test_seeds,
                  episodes_before_train=EPISODES_BEFORE_TRAIN, traffic_density=traffic_density,
                  pop_size=0, rollout_size=0, dirs=None)

    # load the model if exist
    my_al.load(model_dir, train_mode=False)
    rewards, _, steps, avg_speeds = my_al.evaluation(output_dir=None,
                                                     policy=my_al.actor,
                                                     env = env_eval,
                                                     eval_episodes=len(seeds),
                                                     is_train=False)

    rewards = [np.sum(i) for i in rewards]
