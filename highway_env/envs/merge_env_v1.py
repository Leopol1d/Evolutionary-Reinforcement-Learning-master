...
class MergeEnv(AbstractEnv):
    ...
    def _reward(self, action: int) -> float: # action为int整形(动作的序号)，返回值为float
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
       ...
       """
        ...
        scaled_speed = utils.lmap(vehicle.speed, self.config["reward_speed_range"], [0, 1]) # r_s = min((v-v_min)/(v_max-v_min), 1)
        # compute cost for staying on the merging lane
        if vehicle.lane_index == ("b", "c", 1):
            Merging_lane_cost = - np.exp(-(vehicle.position[0] - sum(self.ends[:3])) ** 2 / ( # r_m
                    10 * self.ends[2]))
        else:
            Merging_lane_cost = 0
        ...
        Headway_cost = np.log(
            headway_distance / (self.config["HEADWAY_TIME"] * vehicle.speed)) if vehicle.speed > 0 else 0 # r_h
