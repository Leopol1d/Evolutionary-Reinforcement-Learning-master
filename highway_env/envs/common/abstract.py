...
class AbstractEnv(gym.Env):
    ...
    def default_config(cls) -> dict:
        ...
        return {
            ...
            "n_step": 8,  # do n step prediction 5
            "seed": 0,
            "action_masking": True
        }
    ...
    def reset(self, is_training=True, testing_seeds=0, num_CAV=4) -> Observation:
        """
    ...
    def _reset(self, num_CAV=4) -> None:
        """
        ...
        for t in range(time_steps + 1):
            ...
            if action == 0 or action == 2:
                for vj in surrounding_vehicles:
                    # vehicles.trajectories[t][0][0]表示在时间步长t的longitude位置。
    ...
    def safety_supervisor(self, actions):
        ...
        n_points = int(self.config["simulation_frequency"] // self.config["policy_frequency"]) * self.config[
            "n_step"] # n step predictions
        """compute the priority of controlled vehicles"""
        q = PriorityQueue() # priority_number小，优先级大
        ...
        for vehicle, action in zip(env_copy.controlled_vehicles, actions):
            ...
            if vehicle.lane_index == ("b", "c", 1):
                ...
                headway_distance = self._compute_headway_distance(vehicle)  # headway_distance与前面车辆的距离
                priority_number += 0.5 * np.log(headway_distance  # headway_time=1.2
